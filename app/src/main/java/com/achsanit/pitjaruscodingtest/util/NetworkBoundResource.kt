package com.achsanit.pitjaruscodingtest.util
...
inline fun <ResultType, RequestType> networkBoundResource(
    ...
    val resource = if (shouldFetch(data)) {
        ...
        try {
            ...
            e.printStackTrace()
            query().map {
                Resource.Error(e.message.toString(), CodeError.NO_INTERNET_CONNECTION, it)
            }
        } catch (e: SocketTimeoutException) {
            e.printStackTrace()
            query().map {
                Resource.Error(e.message.toString(), CodeError.REQUEST_TIME_OUT, it)
            }
        } catch (e: Exception) {
            e.printStackTrace()
            query().map {
